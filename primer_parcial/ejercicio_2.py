...
class Superheroe:
    ...
    def __str__(self):
        return f"{self.name} ({self.alias}) - {self.real_name} - {self.first_appearance} - {'Villano' if self.is_villain else 'Héroe'}"

# defino la lista
heroes = List()

# cargo los datos como objetos Superheroe en la lista heroes
for h in superheroes:
    heroe = Superheroe(h["name"], h["alias"], h["real_name"], h["short_bio"], h["first_appearance"], h["is_villain"])
    heroes.append(heroe)

# barrido para chequear
# heroes.show()

# defino los criterios que necesito
def by_name(hero):
    return hero.name

def by_real_name(hero):
    return hero.real_name or "" # "real_name": None

def by_first_appearance(hero):
    return hero.first_appearance

# agregamos esos criterios a la lista
heroes.add_criterion("name", by_name)
heroes.add_criterion("real_name", by_real_name)
heroes.add_criterion("first_appearance", by_first_appearance)
...
# Determinar en que posicion esta The Thing y Rocket Raccoon.
pos_thing = heroes.search("The Thing", criterion_key="name")
pos_rocket = heroes.search("Rocket Raccoon", criterion_key="name")
...
# Listar los superheores que comienzan con  Bl, G, My, y W.
print()
print("Superhéroes que comienzan con Bl, G, My o W:")
for heroe in heroes:
    if heroe.name.startswith(("Bl", "G", "My", "W")):
        print(heroe)

# Listado de personajes ordenado por nombre real de manera ascendente de los personajes.
print()
print("Listado de personajes ordenado por nombre real de manera ascendente:")
heroes.sort_by_criterion("real_name")
heroes.show()
...
# Modificar el nombre real de Ant Man a Scott Lang.
print()
ant_man = heroes.search("Ant Man", "name")
if ant_man is not None:
    heroes[ant_man].real_name = "Scott Lang"

# Mostrar los personajes que en su biografia incluyan la palabra time-traveling o suit.
print()
print("Personajes que en su biografía incluyen 'time-traveling' o 'suit':")
for heroe in heroes:
    if any(palabra in heroe.short_bio.lower() for palabra in ["time-traveling", "suit"]):
        print(heroe)

# Eliminar a Electro y Baron Zemo de la lista y mostrar su información si estaba en la lista.
print()
for nombre in ["Electro", "Baron Zemo"]:
    eliminado = heroes.delete_value(nombre, "name")
